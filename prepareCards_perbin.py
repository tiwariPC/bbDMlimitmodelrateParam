...
f  = open('datacard_templates/datacard_template_'+category+'_perBin.yaml')
...
#    top_temp += 'shapes * '+reg+'_bin'+ibin+' AllMETHistos.root  bbDM'+year+'_'+cat+'_'+reg+'_bin'+ibin+'_$PROCESS bbDM'+year+'_'+cat+'_'+reg+'_bin'+ibin+'_$PROCESS_$SYSTEMATIC'
#    return top_

def getUpperPart2(reg,cat):
   top_ = ''
   for ibin in range(1,5):
      # top_ += '\n shapes * bin'+str(ibin)+'_'+reg+' AllMETHistos.root  bbDM'+year+'_'+cat+'_'+reg+'_bin'+str(ibin)+'_$PROCESS bbDM'+year+'_'+cat+'_'+reg+'_bin'+str(ibin)+'_$PROCESS_$SYSTEMATIC'
      top_ += '\n shapes * '+reg+'_bin'+str(ibin)+' AllMETHistos.root  bbDM'+year+'_'+cat+'_'+reg+'_bin'+str(ibin)+'_$PROCESS bbDM'+year+'_'+cat+'_'+reg+'_bin'+str(ibin)+'_$PROCESS_$SYSTEMATIC'
   return top_
# def getUpperPart2(reg,cat):
...
def getUpperPart3(reg):
   ...
   # top_temp = ''
   # for ibin in range(1,5):
   #    # top_temp += ('bin'+str(ibin)+'_'+reg+' ').lower()
   #    top_temp += (reg+'_bin'+str(ibin)+' ').lower()
   #    # observation -1
   return top_+top_temp
...
for reg in regions:
   ...
   if reg=='SR':
      ...
      for sigHist in getSignalHists(signalDoc,modelName):
         ...
         for i in range(0,len(df0.T.iloc[0]),4):
            k=1
            for j in range(i,i+4):
               # df0.T.iloc[0][j] = ('bin'+str(k)+'_'+df0.T.iloc[0][j]).lower()
               df0.T.iloc[0][j] = (df0.T.iloc[0][j]+'_bin'+str(k)).lower()
               k+=1
         p0 = df0.T.to_string(justify='right',index=True, header=False)
         # p1 = df1.T.to_string(justify='right',index=True, header=False)
         p = df.T.to_string(justify='right',index=True,header=False)
         ...
         fout.write('------------'+'\n')
         fout.write(part3+'\n')
         fout.write('observation -1 -1 -1 -1'+'\n')
         fout.write('------------'+'\n')
         fout.write(p0+'\n')
         # fout.write(p1+'\n')
         fout.write('------------'+'\n')
...
for sigHist in getSignalHists(signalDoc,modelName):
   ...
   if category=='1b':
      ...
      for ibin in ['bin1','bin2','bin3','bin4']:
         ftmp.write('\n')
         ftmp.write("ratewjets_"+category+"_"+year+"_"+ibin+"        rateParam we "+ibin+"_wjets 1. [0.5,1.5]"+"\n")
         ftmp.write("ratewjets_"+category+"_"+year+"_"+ibin+"        rateParam wmu "+ibin+"_wjets 1. [0.5,1.5]"+"\n")
         ftmp.write("ratewjets_"+category+"_"+year+"_"+ibin+"        rateParam sr "+ibin+"_wjets 1. [0.5,1.5]"+"\n")
         #ftmp.write("ratewjets_m        rateParam sr wjets 1. [0.5,1.5]"+"\n")
         ftmp.write("ratezjets_"+category+"_"+year+"_"+ibin+"     rateParam zee "+ibin+"_dyjets 1. [0.5,1.5]"+"\n")
         ftmp.write("ratezjets_"+category+"_"+year+"_"+ibin+"     rateParam zmumu "+ibin+"_dyjets 1. [0.5,1.5]"+"\n")
         ftmp.write("ratezjets_"+category+"_"+year+"_"+ibin+"     rateParam sr "+ibin+"_zjets 1. [0.5,1.5]"+"\n")
         #ftmp.write("ratezjets_m     rateParam sr zjets 1. [0.5,1.5]"+"\n")
      ftmp.write('\n')
      ftmp.write("sr autoMCStats 100 0 1"+"\n")
      ftmp.write("we autoMCStats 100 0 1"+"\n")
      ftmp.write("wmu autoMCStats 100 0 1"+"\n")
      ftmp.write("zee autoMCStats 100 0 1"+"\n")
      ftmp.write("zmumu autoMCStats 100 0 1"+"\n")
      ftmp.close()
      ...
      for ibin in ['bin1','bin2','bin3','bin4']:
         ftmp.write('\n')
         ftmp.write("ratett_"+category+"_"+year+"_"+ibin+"        rateParam tope "+ibin+"_tt  1. [0.5,1.5]"+"\n")
         ftmp.write("ratett_"+category+"_"+year+"_"+ibin+"        rateParam topmu "+ibin+"_tt  1. [0.5,1.5]"+"\n")
         ftmp.write("ratett_"+category+"_"+year+"_"+ibin+"        rateParam sr "+ibin+"_tt  1. [0.5,1.5]"+"\n")
      ...
      ftmp.write('\n')
      ftmp.write("sr autoMCStats 100 0 1"+"\n")
      ftmp.write("tope autoMCStats 100 0 1"+"\n")
      ftmp.write("topmu autoMCStats 100 0 1"+"\n")
      ftmp.write("zee autoMCStats 100 0 1"+"\n")
      ftmp.write("zmumu autoMCStats 100 0 1"+"\n")
      ftmp.close()
      ftxt.write(outdir+'/'+outfile+' \n')
